...
import { createQuote, getCustomers, getProducts, getCompanySettings } from '@/lib/database'
...
export default function NeuesAngebotPage() {
  ...
  useEffect(() => {
    if (!validUntil) {
      ...
      valid.setDate(valid.getDate() + 30) // Angebot 30 Tage gültig
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const quoteItems: Partial<InvoiceItem>[] = items.map(item => ({
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500">Angebot erstellen</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Angebotsdatum</label>
              <input
                type="date"
                value={issueDate}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <button type="button" onClick={addItem} className="flex items-center gap-2 text-sm text-green-600 hover:text-green-700">
              <Plus size={18} />
              Position hinzufügen
            </button>
          </div>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {products.length > 0 && (
                    <select
                      ...
                      className="flex-1 px-3 py-1 text-sm border border-slate-200 rounded-lg"
                    >
                      <option value="">Aus Produktkatalog wählen...</option>
                      {products.map(p => (
                        <option key={p.id} value={p.id}>{p.name} - {Number(p.price).toLocaleString('de-DE')} €</option>
                      ))}
                    </select>
                  )}
                  {items.length > 1 && (
        ...
        <div className="flex justify-end gap-4">
          <Link href="/angebote" className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg">
            Abbrechen
          </Link>
          <button
            type="submit"
